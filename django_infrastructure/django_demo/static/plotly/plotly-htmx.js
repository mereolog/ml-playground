...
function htmlUnescape(input) {
  ...
  htmx.defineExtension('htmx-plotly', {
      transformResponse : (text, xhr, elt) => {
        ...
        //   const restyleLayout = JSON.parse(payload['restyle_layout'])

        const restyleLayout = JSON.parse('{"title": "Linear Function: y = mxx + b"}');
          ...
          if (plotEl) {
              ...
              var plotDiv = document.getElementById(plotId); // div element pointed at
              if (plotDiv) {
                
                  // https://plotly.com/javascript/plotlyjs-function-reference/#plotlynewplot
                  const restyleLayout = plotDiv.layout
                    console.log(restyleData)
                  Plotly.react(plotDiv, restyleData, restyleLayout)
