...
st.markdown(
    "<h1 style='text-align: center; color: #006699;'>Regresja liniowa</h1>",
...
if uploaded_file:
    data, x_values, y_values = load_data(uploaded_file)

    with st.container():
        st.markdown(
            "<h3 style='color: #003366;'>Podgląd danych:</h3>", unsafe_allow_html=True
        )  # navy blue color
        st.dataframe(
            pd.DataFrame({
                "Doświadczenie": x_values.flatten(),
                "Wynagrodzenie": y_values.astype(str),
            }),
            height=150,
        )
    ...
    with col1:
        regularization_type = st.selectbox(
            "Wybierz typ regularyzacji", ["None", "Lasso", "Ridge"]
        )
    ...
    if "YearsExperience" not in data.columns or "Salary" not in data.columns:
        ...
        y_test, y_pred = train_model(x_values, y_values, regularization_type, alpha)
        ...
        if y_test is not None and y_pred is not None:
            ...
            fig.update_layout(
                title="Rzeczywiste vs Przewidywane wyniki",
                xaxis_title="Rzeczywiste wynagrodzenie",
                yaxis_title="Przewidywane wynagrodzenie",
            )
            st.plotly_chart(fig)

    plot_training_steps(x_values, y_values, learning_rate, epochs, cost_function)
