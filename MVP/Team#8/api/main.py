...
class DataProcessor:
    ...
    @staticmethod
    def prepare_data(
        csv_content: str, dependent_variable: str, independent_variable: str
        ...
        dependent_data = (
            torch.from_numpy(df[dependent_variable].values).unsqueeze(1).float()
        )
        independent_data = (
            torch.from_numpy(df[independent_variable].values).unsqueeze(1).float()
        )

        return DataProcessor.normalize_data(
            dependent_data
        ), DataProcessor.normalize_data(independent_data)
        ...
        if data["task"] == "prepare_model":
            ...
            response_data = {
                ...
                "dependent_variable": DataProcessor.normalize_data(
                    torch.from_numpy(dependent_variable_data)
                ).tolist(),
                "independent_variable": DataProcessor.normalize_data(
                    torch.from_numpy(independent_variable_data)
                ).tolist(),
            ...
            for epoch in range(epochs):
                ...
                mae_values.append(
                    MetricsCalculator.mean_absolute_error(y_data_norm, pred_y)
                )
                mse_values.append(
                    MetricsCalculator.mean_squared_error(y_data_norm, pred_y)
                )
                rmse_values.append(
                    MetricsCalculator.root_mean_squared_error(y_data_norm, pred_y)
                )
